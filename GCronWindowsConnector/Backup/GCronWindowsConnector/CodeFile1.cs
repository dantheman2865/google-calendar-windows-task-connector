    ...
    static void Main(string[] args)
    {
        ...
        while (fileinput.EOF == false)
        {
            if (fileinput.NodeType.Equals(XmlNodeType.Element))
            {
                ...
                {
                    case "username":
                        user = DPAPI.Decrypt(fileinput.ReadElementContentAsString());
                        break;
                    case "password":
                        pass = DPAPI.Decrypt(fileinput.ReadElementContentAsString());
                        break;
                    case "calendarURI":
                        feedUrl = DPAPI.Decrypt(fileinput.ReadElementContentAsString());
                        break;
                    case "windowsUsername":
                        windowsUsername = DPAPI.Decrypt(fileinput.ReadElementContentAsString());
                        break;
                    case "windowsPassword":
                        windowsPassword = DPAPI.Decrypt(fileinput.ReadElementContentAsString());
                        break;
                    case "daysAhead":
        ...
        using (TaskService ts = new TaskService())
        {
            ...
            foreach (EventEntry eventEntry in myResultsFeed.Entries)
            {
                ...
                if (commands.Length >= 1)
                {
                    ...
                    td.Principal.UserId = windowsUsername;
                    ...
                    gtf.RegisterTaskDefinition(filename, td,TaskCreation.Create,windowsUsername,windowsPassword);
                }
            }
        }
    }
